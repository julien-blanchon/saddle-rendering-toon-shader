...
```rust,no_run
use bevy::prelude::*;
use saddle_rendering_toon_shader::{ToonExtension, ToonShaderPlugin};
...
```rust,no_run
use bevy::prelude::*;
use saddle_rendering_toon_shader::ToonExtension;
...
- Direct helpers and scene replacement force `OpaqueRendererMethod::Forward`, because the stylized banding depends on post-lighting data that Bevy's deferred prepass does not expose.
...
```bash
TOON_SHADER_AUTO_EXIT_SECONDS=2 cargo run -p saddle-rendering-toon-shader-example-basic
